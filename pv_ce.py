#====================Carnot Cycle=====================#
# 1. Isentropic Compression T2 to T1, V1 to V2
# 2. Isothermal Expansion T2, V2 to V1
# 3. Isentropic Expansion T2 to T1, V1 to V3
# 4. Isothermal Compression T1, V3 to V1

# ce_defmfact_0.5_rho_0.2_eps_1.0.in
...
for q in range(file_count1,file_count2+1):
    ...
    with open('ce_isen_comp_pvte_defmfact_'+str(defmfact)+'_rho_' + str(rho) +'_eps_'+str(eps)+'_cyc_'+str(q)+'_dat.out') as f1: # output file open
        lines1 = f1.readlines()
    
    with open('ce_isoth_exp_pvte_defmfact_'+str(defmfact)+'_rho_' + str(rho) +'_eps_'+str(eps)+'_cyc_'+str(q)+'_dat.out') as f2: # output file open
        lines2 = f2.readlines()
        
    with open('ce_isen_exp_pvte_defmfact_'+str(defmfact)+'_rho_' + str(rho)+'_eps_'+str(eps)+'_cyc_'+str(q)+'_dat.out') as f3: # output file open
        lines3 = f3.readlines()
    
    with open('ce_isoth_comp_pvte_defmfact_'+str(defmfact)+'_rho_' + str(rho) +'_eps_'+str(eps)+'_cyc_'+str(q)+'_dat.out') as f4: # output file open
        lines4 = f4.readlines()
    ...
    z1 = np.polyfit(v1, p1, 2)
    tl1 = z1[0]*v1*v1 + z1[1]*v1 + z1[2]
    
    z2 = np.polyfit(v2, p2, 2)
    tl2 = z2[0]*v2*v2 + z2[1]*v2 + z2[2]
    
    z3 = np.polyfit(v3, p3, 2)
    tl3 = z3[0]*v3*v3 + z3[1]*v3 + z3[2]
    
    z4 = np.polyfit(v4, p4, 2)
    tl4 = z4[0]*v4*v4 + z4[1]*v4 + z4[2]
    ...
    #
    #print('Carnot Efficiency: ' + str(eff_car))
    ...
    #    plt.title('PV diagram of of proposed Carnot Engine with defmfact ' + str(defmfact) + ' rho ' +str(rho) +' eps '+ str(eps))
    #    plt.savefig('PV diagram of of proposed Carnot Engine with defmfact ' + str(defmfact) + ' rho ' +str(rho) +' eps '+ str(eps)+'.png', dpi = 500)
    #    #plt.show()
