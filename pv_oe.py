...
# 2. Isochoric Addition
# 3. Isentropic Expansion
# 4. Isochoric Heat Extraction

# oe_defmfact_0.5_rho_0.2_eps_1.0.in
...
for q in range(file_count1,file_count2+1):
    ...
    with open('oe_isen_comp_pvte_defmfact_'+str(defmfact)+'_rho_' + str(rho) +'_eps_'+str(eps)+'_cyc_'+str(q)+'_dat.out') as f1: # output file open
        lines1 = f1.readlines()
    
    with open('oe_isoch_heatadd_pvte_defmfact_'+str(defmfact)+'_rho_' + str(rho) +'_eps_'+str(eps)+'_cyc_'+str(q)+'_dat.out') as f2: # output file open
        lines2 = f2.readlines()
        
    with open('oe_isen_exp_pvte_defmfact_'+str(defmfact)+'_rho_' + str(rho)+'_eps_'+str(eps)+'_cyc_'+str(q)+'_dat.out') as f3: # output file open
        lines3 = f3.readlines()
    
    with open('oe_isoch_heatsub_pvte_defmfact_'+str(defmfact)+'_rho_' + str(rho) +'_eps_'+str(eps)+'_cyc_'+str(q)+'_dat.out') as f4: # output file open
        lines4 = f4.readlines()
    ...
    #print('\n================='+str(q)+' Cycle================\n')
    #
    #print('Work done in Isentropic Compresssion: ' + str(work1))
    #print('Work done in Isochoric Heat Addition: ' + str(work2))
    #print('Work done in Isentropic Expansion: ' + str(work3))
    #print('Work done in Isochoric Heat Extraction: ' + str(work4))
    ...
    eff_ideal = 1 - ((T3 - T_init)/(T2 - T1))
    eff_here = tot_work/posititve_heat
    
    ideal_eff[scount] = eff_ideal
    ...
    #print(T_init,T1,T2,T3)
    #print('Posititve Heat provided: ' + str(posititve_heat))
    #
    #print('Ideal Efficiency: ' + str(eff_ideal))
    #print('Efficiency Here: ' + str(eff_here))
    
    if q==1:
        plt.plot(v1,p1,v2,p2,v3,p3,v4,p4)
        plt.legend(['Isentropic Compression','Isochoric Heat Addition', 'Isentropic Expansion', 'Isochoric Heat Rejection'])
        plt.ylabel('Pressure in x-direction [LJ Units]')
        plt.xlabel('Volume [LJ Units]')
        plt.title('PV diagram of of proposed Otto Engine with defmfact ' + str(defmfact) + ' rho ' +str(rho) +' eps '+ str(eps))
        plt.savefig('PV diagram of of proposed Otto Engine with defmfact ' + str(defmfact) + ' rho ' +str(rho) +' eps '+ str(eps)+'.png', dpi = 500)
        #plt.show()
...
#        print(str(proposed_eff[i]), ideal_eff[i])
#    sys.stdout = original_stdout
#
#plt.plot(q_count, (1 - (T2/T1))*np.ones(len(q_count)),q_count,proposed_eff)
#plt.legend(['Fixed Ideal Carnot Efficiency','Calculated Efficiency'])
#plt.ylabel('Efficiency')
#plt.xlabel('No. of Cycles')
#plt.title('Cycle Efficiency of Carnot Engine with defmfact ' + str(defmfact) + ' rho ' +str(rho) +' eps '+ str(eps) + ' with ' + str(scount) + ' cycles')
#plt.savefig('Cycle Efficiency of Carnot Engine with defmfact ' + str(defmfact) + ' rho ' +str(rho) +' eps '+ str(eps) + ' with ' + str(scount) + ' cycles' + '.png', dpi = 500)